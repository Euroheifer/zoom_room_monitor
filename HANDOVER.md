...
| Trigger actions (one per scope) | CNGR **280**, SG **281**, SG-GLX **282**, SG-RC **283**, SG-5SPD **284**, plus BR-FLP / BR-B32 — host group + severity ≥ Average (+ tag conditions for building scopes), problem + recovery. A scope's tag values may be a LIST: every type-26 condition is OR'd by evaltype 0, which is how `BR-B32` covers B32 + HYP + FBSSP9 + SFB and, via `role=summary`, the BR collector watchdog |
