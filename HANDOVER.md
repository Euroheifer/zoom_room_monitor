...
| Grafana dashboards | company Grafana: **fleet, all regions (`zoom-fleet`)** — `$region`/`$building` single-select, `$floor` multi; per-region detail in a collapsed repeated row (repeat still works with `$region=All`). Top section is the per-region offline tiles + a fleet-wide issues table (which alone shows collector watchdogs, host filter `/.*/`); fleet totals were removed 2026-09-03 as they duplicated the selected region's own panels. The room grid strips the `REGION-building-floor-` prefix via `renameByRegex` so tile text stays readable — its drill-down link uses `${__field.labels.host}`, which is unaffected by the rename; room detail (`zoom-room-detail`, region-agnostic); superseded per-region copies `zoom-sg-poc` / `zoom-cngr-poc` kept until the fleet view is proven. Import via `deploy/upload-to-grafana/` symlinks (UI upload, overwrite) |
...
| Force a test alert | push `zoom.room.issues` = `TEST ALERT please ignore - ...` then `none` (the issue trigger flips on one value; offline needs two). **Pick a room whose issues are currently `none`** — a room already in problem state just changes the value, so no new event and no alert. Recovery messages only fire for problems that *started* after the action existed. **Always print the `history.push` response and poll the trigger value** instead of sleeping: a rejected push otherwise looks identical to a delivered alert, and chasing that costs a session. Verify with `alert.get` (`status 1` = sent, plus the `sendto` URL) rather than by watching the chat. |
